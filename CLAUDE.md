...
- **SerpAPI circuit breaker**: retrying is only safe while failures are rare. On 2026-09-05 SerpAPI stopped answering entirely, every Google News query burned its full retry budget (~141s each at the defaults), and the daily run was killed at the 20-minute Actions ceiling having produced nothing — the same outage killed `new-scientist-story-ideas` at its 30-minute ceiling the same morning. `_serp_get()` now counts consecutive failed *queries* and, past `SERPAPI_FAILURE_THRESHOLD` (default 3), returns `None` immediately for the rest of the process. **Count queries, not attempts** — the first version counted attempts, and on the 2026-09-06 prefetch run a single slow query (`medical study when:7d`) burned its three attempts while every other query succeeded, tripped the breaker, and starved Google Trends of its entire budget (0 seed keywords, and Trends is 30% of `trend_strength_score`). One bad query is not an outage; three consecutive failed queries is. A success resets the count. **Count per engine, not globally** — with the per-query fix in place, prefetch run 34001126950 hit three genuinely dead Google News queries and opened the single shared breaker *before Google Trends made one call*, producing 0 seed keywords a second time from an outage never shown to apply to Trends. Each engine group now keeps its own tally and its own `SERPAPI_ENGINE_TIME_BUDGET_SECONDS` (default 240); the three `google_trends*` engines share a group because they share an upstream. `SERPAPI_TIME_BUDGET_SECONDS` (default 600) is a wall-clock backstop for the slow-but-succeeding case the failure count never sees, and outranks the per-engine budgets, so a true total outage stops everything (~12 attempts, ~9 minutes, verified against the 20-minute ceiling). A plain 4xx does not count toward it — that is a malformed request, returns instantly, and costs no budget. `google_trend_radar.py` carries the same guard (threshold 5) since a radar run can issue 40+ Trends calls. **The breaker changes what a thin run means**: `build_serp_context()` prepends a `SIGNAL INTEGRITY WARNING` block when it trips, so absence of signal is read as unknown rather than as a quiet news week. A partial run beats a killed one, but only if the report says which it was.
