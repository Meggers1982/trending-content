...
Reddit's anonymous JSON API is rate-limited and can be flaky; `discover_reddit_candidates()` fails soft (logs to the existing `REQUEST_FAILURES` counter, never raises) and `--skip-reddit` disables it entirely if needed.
