...
export const TRACKED_TOPICS_UNAVAILABLE_MESSAGE =
  "Tracked topics are unavailable because DATABASE_URL is not configured.";

let sqlClient = null;

export function isTrackedTopicsConfigured() {
  return Boolean(process.env.DATABASE_URL);
}

function getSql() {
  if (!isTrackedTopicsConfigured()) {
    throw new Error(TRACKED_TOPICS_UNAVAILABLE_MESSAGE);
  }
  if (!sqlClient) {
