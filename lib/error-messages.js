export function explainRunError(rawError, mode = "full") {
  ...
  const action = mode === "prefetch" ? "refresh the radar" : "start a new report";
  ...
  if (lower.includes("workflow dispatch failed") && (lower.includes("401") || lower.includes("403"))) {
    return "GitHub rejected the pipeline token. Create a fine-grained token with Actions read/write access for Meggers1982/trending-content, then update GITHUB_PIPELINE_TOKEN in Vercel.";
  }
